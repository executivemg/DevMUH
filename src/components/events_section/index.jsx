...
const EventSection = ({ className }) => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const response = await axios.get("http://127.0.0.1:8000/ranaevents/events");
  ...
  return (
    <section className={`flex flex-col gap-[1rem] justify-center w-full ${className}`}>
      ...
      <div className="flex justify-center w-full ">
        <button className="flex text-center justify-center items-center w-[12rem] py-[0.5rem] rounded-[8px] text-[0.875rem] text-white bg-base hover:bg-primary outline outline-base-light outline-0 hover:outline-[1px] shadow-[0_2px_15px_5px_rgba(150,150,255,0.1)] hover:shadow-[0_2px_15px_10px_rgba(150,150,255,0.2)] transition-all ease-in-out duration-[1s]">
          View All
        </button>
      </div>
    </section>
