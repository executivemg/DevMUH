...
const p = `text-gray-600 font-semibold`;
      ...
      <div className='sm:w-[80vw] w-[90vw] 2xl:w-[1500px] flex md:justify-between md:flex-row flex-col gap-4'>
        <div className='md:w-[35%] w-full'>
          ...
          <h1 className='mt-4 text-5xl font-bold'>
            <span className='text-white'>EVENT </span> <span className='bord'>RUSH</span>
          </h1>
          <p className={`${p} my-4`}>
        ...
        <div className='relative md:w-[55%] w-full'>
          <div className='w-full h-full rounded-3xl bg-gradient-to-r from-[#2C3BFA] to-[#B10C61]'>
            <video /* poster='/thumbnail.png' */ ref={videoRef} autoPlay={playing} muted loop className="w-full h-full object-cover rounded-3xl md:absolute mt-8 md:mt-0 md:bottom-14 md:right-14 bottom-0 right-0 z-40">
              <source src='/videocont.mp4' type="video/mp4" />
            </video>
            {!playing && <div className='absolute md:bottom-14 md:right-14 -bottom-8 z-40 w-full h-full flex justify-center items-center'>
              <div className='backdrop-blur-3xl p-14 rounded-full'>
                <img src="/rotate.png" className='absolute top-1 left-1 w-[165px] h-[165px] rotate -z-50' alt="" />
                <div className='bg-white p-4 rounded-full cursor-pointer z-40' onClick={togglePlay}>
                  {!playing ? <PlayArrow className='' fontSize='large' /> : <Pause fontSize='large' />}
                </div>
              </div>
            </div>}
          </div>
